...
RESULT_DIR="${RESULT_DIR:-results/active_counts}"
...
make clean
make PROFILE_ACTIVE_COUNTS=1

for neutrons in $NEUTRON_COUNTS; do
    csv="$RESULT_DIR/active_counts_N${neutrons}_G${GENERATIONS}_B${BATCH_SIZE}.csv"
    log="$RESULT_DIR/active_counts_N${neutrons}_G${GENERATIONS}_B${BATCH_SIZE}.log"
